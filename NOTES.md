...
### 2.37 Phase 14: three lists in somebody else's cloud
...
10. **The Exchange admin API is beta and undocumented.**
    ...
    list from PHP — Graph will not (§2.37). Everything the connector believes
...
11. **`New-MailContact` is assumed to fail on a duplicate `Name`.** The
...
12. **A failed add or remove is checked by reading the membership back.** This
    ...
    than a chosen few.
13. **Three attempts, ten minutes apart, one row per request.** All arbitrary,
